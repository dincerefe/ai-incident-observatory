...
async function startServer() {
  try {
    // 1. Give DB a moment to run initial schema scripts, then count rows
    setTimeout(async () => {
      try {
        ...
        // Load ethics questions + AI answers from CSV if not yet in DB
        ...
        if (process.env.SYNC_ON_STARTUP === 'true') {
          ...
          // Execute asynchronously so the server starts listening instantly
          syncAll()
            .then(stats => {
        ...
        console.error('Startup database verification failed:', err.message);
      }
    }, 1000);
